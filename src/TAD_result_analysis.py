...
def detect_anomalies(model, dataset, meta_df, threshold_df=None, threshold=0.0, base_dim=3):
    ...
    if threshold_df is not None:
        ...
        result_df['anomaly'] = (result_df['error'] > result_df['Thresholds_applied']).astype(int)
    
    else:
        # 단일 임계값 적용 (예: Validation Reconstruction Error 계산 시)
        result_df['Thresholds_applied'] = threshold 
        result_df['anomaly'] = (result_df['error'] > threshold).astype(int) 

    return result_df[['TOT_DT', 'LINK_ID', 'LANE_NO', 'error', 'Thresholds_applied', 'anomaly', 'pred']]
